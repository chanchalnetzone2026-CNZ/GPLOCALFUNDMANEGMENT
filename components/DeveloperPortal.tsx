...
export const DeveloperPortal: React.FC<DeveloperPortalProps> = ({
  ...
  const multiPanchayatStats = useMemo(() => {
    const normalizeRegional = (val?: string) => {
      if (!val) return '';
      return val
        .toLowerCase()
        .replace(/(कार्यालय\s*जनपद\s*पंचायत|जनपद\s*पंचायत|कार्यालय\s*ग्राम\s*पंचायत|ग्राम\s*पंचायत|कार्यालय|ब्लॉक|block|janpad\s*panchayat|janpad|जिला|district|office\s*gram\s*panchayat|gram\s*panchayat|panchayat|gp)/gi, '')
        .replace(/[^a-z0-9\u0900-\u097F]/gi, '')
        .trim();
    };

    if (!adminList || adminList.length === 0) {
      return { registeredPanchayatsCount: 0, registeredBlocksCount: 0, registeredDistrictsCount: 0 };
    }
    ...
    const sharedDistrict = adminList.find(a => (a.district || '').trim())?.district?.trim() || 'district_main';
    const sharedBlock = adminList.find(a => (a.block || '').trim())?.block?.trim() || 'block_main';

    adminList.forEach((a) => {
      const cleanGp = normalizeRegional(a.gramPanchayat) || a.gramPanchayat?.trim().toLowerCase() || a.id;
      if (cleanGp) panchayatSet.add(cleanGp);

      const finalDist = (a.district || '').trim() || sharedDistrict;
      const cleanDist = normalizeRegional(finalDist) || 'district_main';
      districtSet.add(cleanDist);

      const finalBlock = (a.block || '').trim() || sharedBlock;
      const cleanBlock = normalizeRegional(finalBlock) || 'block_main';
      blockSet.add(`${cleanDist}::${cleanBlock}`);
    });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-2">
                ...
                <div className="pt-2 border-t border-slate-100 flex justify-between text-[11px] text-slate-600 font-medium">
                  <span>{isHindi ? 'ब्लॉक:' : 'Blocks:'} <strong className="text-emerald-700 font-bold">{multiPanchayatStats.registeredBlocksCount}</strong></span>
                  <span>{isHindi ? 'जिले:' : 'Districts:'} <strong className="text-indigo-700 font-bold">{multiPanchayatStats.registeredDistrictsCount}</strong></span>
                  <span>Sub: <strong className="text-primary font-bold">{subscribedCount}</strong></span>
                </div>
              </div>
