...
// ==============================================================================
// 5. OFFICE DETAILS & BANK PROFILE (STRICT PANCHAYAT ISOLATION)
// ==============================================================================

export async function fetchAllOfficeDetailsFromSupabase(): Promise<OfficeDetails[] | null> {
  const supabase = getSupabaseClient();
  if (!supabase) return null;
  try {
    const { data, error } = await supabase.from('office_details').select('*');
    if (error || !data) return null;
    return data.map((row: any) => ({
      officeName: row.office_name || row.panchayat_name || '',
      secretaryName: row.secretary_name || '',
      secretaryDesignation: row.secretary_designation || 'ग्राम पंचायत सचिव',
      sarpanchName: row.sarpanch_name || '',
      contactPhone: row.contact_phone || row.office_mobile || '',
      email: row.email || row.office_email || '',
      address: row.address || '',
      block: row.block || row.block_name || '',
      district: row.district || row.district_name || '',
      state: row.state || row.state_name || 'मध्य प्रदेश',
      pincode: row.pincode || '',
    ...
    console.warn('Error fetching all office details from Supabase:', err);
    return null;
  }
}
...
export async function fetchOfficeDetailsFromSupabase(adminId?: string, gramPanchayat?: string): Promise<OfficeDetails | null> {
  ...
  try {
    ...
    if (!data && gramPanchayat && gramPanchayat.trim()) {
    ...
    // If no specific match is found for this admin / panchayat, return null (never leak other panchayats' data)
    if (!data) return null;

    return {
      officeName: data.office_name || data.panchayat_name || '',
      secretaryName: data.secretary_name || '',
      secretaryDesignation: data.secretary_designation || 'ग्राम पंचायत सचिव',
...
export async function saveOfficeDetailsToSupabase(details: OfficeDetails, adminId?: string, gramPanchayat?: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const primaryId = adminId || details.adminId || (details.gramPanchayat ? `office_${details.gramPanchayat.trim().toLowerCase().replace(/\s+/g, '_')}` : `office_${Date.now()}`);
    ...
    if (finalQrUrl && finalQrUrl.startsWith('data:image')) {
      try {
    ...
    const payload = {
      ...
      secretary_designation: details.secretaryDesignation || 'ग्राम पंचायत सचिव',
    ...
    // Upsert record strictly by unique id without overwriting other panchayats
