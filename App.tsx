...
import {
  UpcomingBookingAlertModal,
  getUpcomingBookingAlerts,
  UpcomingBookingAlertInfo,
} from './components/UpcomingBookingAlertModal';
...
const App: React.FC = () => {
  ...
  const [officeDetailsList, setOfficeDetailsList] = useState<OfficeDetails[]>(() => {
    try {
      const savedList = localStorage.getItem('gp_office_details_list');
      if (savedList) {
        const parsed = JSON.parse(savedList);
        if (Array.isArray(parsed)) return parsed;
      }
      const single = localStorage.getItem('gp_office_details');
      if (single) {
        const parsed = JSON.parse(single);
        if (parsed && (parsed.adminId || parsed.gramPanchayat)) {
          return [parsed];
        }
      }
    } catch (e) {}
  ...
  // Upcoming Booking Alert States
  const [isUpcomingAlertModalOpen, setIsUpcomingAlertModalOpen] = useState<boolean>(false);
  const [hasAutoShownBookingAlert, setHasAutoShownBookingAlert] = useState<boolean>(false);
  const [selectedBookingForPrintInApp, setSelectedBookingForPrintInApp] = useState<BookingRentRecord | null>(null);
  ...
  useEffect(() => {
    ...
    // Reset activity timers upon active session
    ...
    const triggerAutoLogout = () => {
      ...
      const sessionExpireNotice = isHindi
        ...
        : '⏱️ Your session was automatically logged out due to 5 minutes of inactivity. Please log in again.';

      setLoginError(sessionExpireNotice);
      showToast(
        isHindi ? '⏱️ निष्क्रियता (5 मिनट) के कारण सत्र स्वतः समाप्त हो गया!' : '⏱️ Logged out due to 5 minutes of inactivity!',
        'warning'
      );
    };
    ...
    const checkInactivity = () => {
      ...
      let storedTime = 0;
      try {
        const item = localStorage.getItem('gp_last_activity_time');
        if (item) storedTime = Number(item);
      } catch (e) {}

      const lastActive = Math.max(lastActivityRef.current, storedTime || 0);
    ...
    // Periodic check every 3 seconds for responsive 5-minute auto-logout
  ...
  const syncAllDataFromSupabase = useCallback(async (isManual = false) => {
    ...
    try {
      const [
        ...
        fetchAllOfficeDetailsFromSupabase(),
  ...
  // Computed Upcoming Booking Alerts (Within 2 Days or Today or Ongoing)
  const upcomingBookingAlerts = useMemo(() => {
    return getUpcomingBookingAlerts(currentBookingRents);
  }, [currentBookingRents]);

  // Auto-show Upcoming Booking Alert Popup on Dashboard if any upcoming booking is found
  useEffect(() => {
    if (
      loggedInAdmin &&
      currentPage === Page.DASHBOARD &&
      upcomingBookingAlerts.length > 0 &&
      !hasAutoShownBookingAlert
    ) {
      setIsUpcomingAlertModalOpen(true);
      setHasAutoShownBookingAlert(true);
    }
  }, [loggedInAdmin, currentPage, upcomingBookingAlerts.length, hasAutoShownBookingAlert]);
  ...
  const currentOfficeDetails = useMemo<OfficeDetails>(() => {
    if (!loggedInAdmin) return initialOfficeDetails;

    const normGp = normalizePanchayatName(loggedInAdmin.gramPanchayat);
    const matching = officeDetailsList.find((o) => {
      if (o.adminId && (o.adminId === loggedInAdmin.id || o.adminId === loggedInAdmin.mobile)) return true;
      if (o.gramPanchayat && normalizePanchayatName(o.gramPanchayat) === normGp) return true;
      return false;
    });

    if (matching) {
      return {
        officeName: (matching.officeName && matching.officeName.trim()) 
          ? matching.officeName 
          : `कार्यालय ग्राम पंचायत ${loggedInAdmin.gramPanchayat}`,
        gramPanchayat: matching.gramPanchayat || loggedInAdmin.gramPanchayat,
        secretaryName: (matching.secretaryName && matching.secretaryName.trim()) 
          ? matching.secretaryName 
          : loggedInAdmin.name,
        secretaryDesignation: matching.secretaryDesignation || loggedInAdmin.designation || 'ग्राम पंचायत सचिव',
        sarpanchName: matching.sarpanchName || '',
        contactPhone: (matching.contactPhone && matching.contactPhone.trim()) 
          ? matching.contactPhone 
          : loggedInAdmin.mobile,
        email: (matching.email && matching.email.trim()) 
          ? matching.email 
          : (loggedInAdmin.email || 'chanchalnetzone2026@gmail.com'),
        address: (matching.address && matching.address.trim()) 
          ? matching.address 
          : `ग्राम पंचायत ${loggedInAdmin.gramPanchayat}, जनपद पंचायत ${matching.block || loggedInAdmin.block || ''}, जिला ${matching.district || loggedInAdmin.district || ''}`,
        block: matching.block || loggedInAdmin.block || '',
        district: matching.district || loggedInAdmin.district || '',
        state: matching.state || loggedInAdmin.state || 'मध्य प्रदेश',
        pincode: matching.pincode || '',
        bankName: matching.bankName || '',
        accountName: matching.accountName || '',
        accountNumber: matching.accountNumber || '',
        ifscCode: matching.ifscCode || '',
        branchName: matching.branchName || '',
        logoUrl: (matching.logoUrl && matching.logoUrl.trim().length > 0) ? matching.logoUrl : 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/Emblem_of_Madhya_Pradesh.svg/180px-Emblem_of_Madhya_Pradesh.svg.png',
        qrCodeUrl: matching.qrCodeUrl || '',
        adminId: loggedInAdmin.id,
      };
    }

    // Default clean baseline for newly logged-in Panchayat (Strict isolation - NO data leakage from other panchayats)
    return {
      officeName: `कार्यालय ग्राम पंचायत ${loggedInAdmin.gramPanchayat}`,
      gramPanchayat: loggedInAdmin.gramPanchayat,
      secretaryName: loggedInAdmin.name || '',
      secretaryDesignation: loggedInAdmin.designation || 'ग्राम पंचायत सचिव',
      sarpanchName: '',
      contactPhone: loggedInAdmin.mobile || '',
      email: loggedInAdmin.email || 'chanchalnetzone2026@gmail.com',
      address: `ग्राम पंचायत ${loggedInAdmin.gramPanchayat}, जनपद पंचायत ${loggedInAdmin.block || ''}, जिला ${loggedInAdmin.district || ''}`,
      block: loggedInAdmin.block || '',
      district: loggedInAdmin.district || '',
      state: loggedInAdmin.state || 'मध्य प्रदेश',
      pincode: '',
      bankName: '',
      accountName: '',
      accountNumber: '',
  ...
  }, [officeDetailsList, loggedInAdmin]);

  // Dynamic Regional Statistics for Registered Panchayats, Blocks & Districts
  // Requirement:
  // - If many Panchayat users register under the same Block -> Block must count 1
  // - If many Gram Panchayats register from different Blocks under the same District -> District must count 1
  const multiPanchayatStats = useMemo(() => {
    if (!adminList || adminList.length === 0) {
      return {
        registeredPanchayatsCount: 0,
        registeredBlocksCount: 0,
        registeredDistrictsCount: 0,
        totalRegisteredAdmins: 0,
      };
    }

    const panchayatSet = new Set<string>();
    const blockSet = new Set<string>();
    const districtSet = new Set<string>();

    // 1. Cross-reference admin records with officeDetailsList, sibling admins, and loggedInAdmin
    const resolvedAdmins = adminList.map((a) => {
      let rawDist = (a.district || '').trim();
      let rawBlock = (a.block || '').trim();
      let rawGp = (a.gramPanchayat || '').trim();

      if (!rawDist || !rawBlock || !rawGp) {
        const matchingOffice = officeDetailsList.find(
          (o) =>
            (o.adminId && (o.adminId === a.id || o.adminId === a.mobile)) ||
            (o.contactPhone && o.contactPhone === a.mobile) ||
            (o.gramPanchayat && a.gramPanchayat && getCanonicalPanchayat(o.gramPanchayat) === getCanonicalPanchayat(a.gramPanchayat))
        );
        if (matchingOffice) {
          if (!rawDist && matchingOffice.district) rawDist = matchingOffice.district.trim();
          if (!rawBlock && matchingOffice.block) rawBlock = matchingOffice.block.trim();
          if (!rawGp && matchingOffice.gramPanchayat) rawGp = matchingOffice.gramPanchayat.trim();
        }
      }

      if (loggedInAdmin && (loggedInAdmin.id === a.id || loggedInAdmin.mobile === a.mobile)) {
        if (!rawDist && loggedInAdmin.district) rawDist = loggedInAdmin.district.trim();
        if (!rawBlock && loggedInAdmin.block) rawBlock = loggedInAdmin.block.trim();
        if (!rawGp && loggedInAdmin.gramPanchayat) rawGp = loggedInAdmin.gramPanchayat.trim();
      }

      return {
        ...a,
        gramPanchayat: rawGp || a.gramPanchayat,
        resolvedDistrict: rawDist,
        resolvedBlock: rawBlock,
      };
    });

    // 2. Identify shared/common district and block across sibling admins if some entries have blanks
    const sharedDistrict =
      resolvedAdmins.find((a) => a.resolvedDistrict)?.resolvedDistrict ||
      loggedInAdmin?.district?.trim() ||
      'NARSINGHPUR';

    const sharedBlock =
      resolvedAdmins.find((a) => a.resolvedBlock)?.resolvedBlock ||
      loggedInAdmin?.block?.trim() ||
      'KARELI';

    resolvedAdmins.forEach((a) => {
      // 1. Gram Panchayat count - deduplicated by canonical name
      const canonGp = getCanonicalPanchayat(a.gramPanchayat) || a.gramPanchayat?.trim().toUpperCase() || a.id;
      if (canonGp) {
        panchayatSet.add(canonGp);
      }

      // 2. District count - deduplicated by canonical district
      const finalDist = a.resolvedDistrict || sharedDistrict;
      const canonDist = getCanonicalDistrict(finalDist) || 'NARSINGHPUR';
      districtSet.add(canonDist);

      // 3. Block count - keyed per district so multiple panchayats in the same block count as 1 block
      const finalBlock = a.resolvedBlock || sharedBlock;
      const canonBlock = getCanonicalBlock(finalBlock) || 'KARELI';
      blockSet.add(`${canonDist}::${canonBlock}`);
    });

    const registeredPanchayatsCount = panchayatSet.size;
    // Mathematical bounds: blocks cannot exceed panchayats, districts cannot exceed blocks
    const registeredBlocksCount = registeredPanchayatsCount > 0 ? Math.min(registeredPanchayatsCount, Math.max(1, blockSet.size)) : 0;
    const registeredDistrictsCount = registeredPanchayatsCount > 0 ? Math.min(registeredBlocksCount, Math.max(1, districtSet.size)) : 0;

    return {
      registeredPanchayatsCount,
      registeredBlocksCount,
      registeredDistrictsCount,
      totalRegisteredAdmins: adminList.length,
    };
  }, [adminList, officeDetailsList, loggedInAdmin]);
  ...
  const handleCreateBookingRent = async (
    ...
    // Trigger upcoming booking alert popup immediately when user books
    setIsUpcomingAlertModalOpen(true);
  ...
  const renderDashboard = () => (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 animate-fade-in max-w-7xl">
      ...
      <div className="mb-6 bg-gradient-to-r from-slate-900 via-primary-950 to-slate-900 text-white p-5 rounded-2xl shadow-xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-white/10 p-2 backdrop-blur border border-white/20 flex items-center justify-center shrink-0">
            <img src={currentOfficeDetails.logoUrl || 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d4/Emblem_of_Madhya_Pradesh.svg/180px-Emblem_of_Madhya_Pradesh.svg.png'} alt="Logo" className="max-h-full max-w-full object-contain" />
          </div>
          <div>
      ...
      {/* UPCOMING BOOKING ALERT PROMINENT DASHBOARD BANNER (BEFORE 2 DAYS TILL BOOKING DATE) */}
      {upcomingBookingAlerts.length > 0 && (
        <div className="bg-gradient-to-r from-amber-500 via-rose-500 to-indigo-600 rounded-2xl p-0.5 shadow-lg animate-fade-in print:hidden">
          <div className="bg-white rounded-[14px] p-4 sm:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-start sm:items-center gap-3.5">
              <div className="w-12 h-12 rounded-2xl bg-amber-500 text-white flex items-center justify-center font-black text-2xl shadow-md shrink-0 animate-pulse">
                🚨
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 bg-rose-100 text-rose-800 text-[10px] font-black rounded-full uppercase border border-rose-300">
                    {isHindi ? '2 दिन पूर्व अलर्ट सूचना' : '2-Day Advance Alert'}
                  </span>
                  <span className="text-xs font-bold text-slate-500">
                    {isHindi
                      ? `कुल ${upcomingBookingAlerts.length} बुकिंग निर्धारित हैं`
                      : `${upcomingBookingAlerts.length} upcoming booking(s) scheduled`}
                  </span>
                </div>
                <h4 className="text-base sm:text-lg font-black text-slate-900 mt-0.5">
                  {isHindi ? '📢 आगामी परिसर / भवन बुकिंग पूर्व सूचना' : 'Upcoming Facility / Hall Booking Alert'}
                </h4>
                <div className="text-xs text-slate-700 font-medium mt-1.5 flex flex-wrap items-center gap-x-2.5 gap-y-1.5">
                  {upcomingBookingAlerts.slice(0, 3).map((item) => (
                    <span
                      key={item.booking.id}
                      className="bg-slate-50 hover:bg-amber-50/60 transition-colors px-3 py-1.5 rounded-xl border border-slate-200 inline-flex items-center gap-1.5"
                    >
                      <span className="font-black text-slate-900">👤 {item.booking.beneficiaryName}</span>
                      <span className="text-slate-500 font-medium">
                        ({item.booking.guardianName || item.booking.fatherHusbandName ? `पिता/पति: ${item.booking.guardianName || item.booking.fatherHusbandName}` : 'वार्ड ' + (item.booking.wardNo || '01')})
                      </span>
                      <span className="text-emerald-800 font-bold ml-1">
                        📅 {formatDateDDMMYYYY(item.booking.startDate)}
                      </span>
                      <span className={`ml-1 px-2 py-0.5 rounded-lg text-[10px] font-black ${item.badgeClass}`}>
                        {item.status === 'TODAY' ? '🔴 आज' : item.status === 'TOMORROW' ? '⚡ कल' : '🚨 2 दिन शेष'}
                      </span>
                    </span>
                  ))}
                  {upcomingBookingAlerts.length > 3 && (
                    <span className="text-xs font-bold text-indigo-700">
                      +{upcomingBookingAlerts.length - 3} {isHindi ? 'अन्य बुकिंग...' : 'more...'}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 self-end md:self-center shrink-0 w-full sm:w-auto justify-end">
              <button
                type="button"
                onClick={() => setIsUpcomingAlertModalOpen(true)}
                className="px-4 py-2.5 bg-gradient-to-r from-amber-600 to-rose-600 hover:from-amber-700 hover:to-rose-700 text-white font-black text-xs rounded-xl shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
              >
                <span>🔔</span>
                <span>{isHindi ? 'अलर्ट पॉपअप देखें' : 'View Alert Popup'}</span>
              </button>
              <button
                type="button"
                onClick={() => {
                  setSelectedBookingForPrintInApp(null);
                  setCurrentPage(Page.BOOKING_RENT);
                }}
                className="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs rounded-xl border border-slate-300 transition-all cursor-pointer"
              >
                {isHindi ? '📋 बुकिंग पंजीयन' : 'View All'}
              </button>
            </div>
          </div>
        </div>
      ...
      case Page.BOOKING_RENT:
        return (
          <BookingRentView
            ...
            isHindi={isHindi}
            initialSelectedBooking={selectedBookingForPrintInApp}
          />
        );
        ...
        return (
          <ManageOfficeView
            ...
            onUpdateOfficeDetails={async (updated) => {
              if (!loggedInAdmin) return;
              const newGp = updated.gramPanchayat || loggedInAdmin.gramPanchayat;
              const updatedRecord: OfficeDetails = {
              ...
              setOfficeDetailsList((prev) => {
                const normGp = normalizePanchayatName(newGp);
                const exists = prev.some(
                  (o) =>
                    (o.adminId && (o.adminId === loggedInAdmin.id || o.adminId === loggedInAdmin.mobile)) ||
                    (o.gramPanchayat && normalizePanchayatName(o.gramPanchayat) === normGp)
                );
                const next = exists
                  ? prev.map((o) =>
                      (o.adminId && (o.adminId === loggedInAdmin.id || o.adminId === loggedInAdmin.mobile)) ||
              ...
              // Also sync updated gp/block/district/state/secretaryName/contactPhone to loggedInAdmin so both are unified
              const updatedAdmin: Admin = {
                ...loggedInAdmin,
              ...
              const success = await saveOfficeDetailsToSupabase(updatedRecord, loggedInAdmin.id, newGp);
              if (success) {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans bg-[#f0f6ff] text-slate-900">
      ...
      {/* UPCOMING BOOKING ALERT MODAL (BEFORE 2 DAYS TILL BOOKING DATE) */}
      <UpcomingBookingAlertModal
        isOpen={isUpcomingAlertModalOpen}
        onClose={() => setIsUpcomingAlertModalOpen(false)}
        alerts={upcomingBookingAlerts}
        onViewBookingPrint={(booking) => {
          setSelectedBookingForPrintInApp(booking);
          setCurrentPage(Page.BOOKING_RENT);
        }}
